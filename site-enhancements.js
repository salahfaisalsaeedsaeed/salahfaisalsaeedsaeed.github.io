...
async function hydrateAssetsWithStorage(data){
  ...
  (data.assets||[]).forEach(asset=>{
    if(asset.file_id && byId.has(String(asset.file_id))){ asset._storage_resolved=true;return; }
  ...
  const resolvedIds=new Set((data.assets||[]).map(a=>String(a.file_id||"")).filter(Boolean));
  data._unlinkedStorageFiles=files.filter(f=>!resolvedIds.has(String(f.$id||"")));
  return data;
}
...
function renderFixedAssetFrame(asset,title){
  ...
  if(!asset?.file_id){
    return `<div class="media-file-placeholder"><span class="media-file-icon">FILE</span><strong>Preview unavailable</strong><span>The database record is preserved, but its Appwrite Storage file could not be resolved.</span></div>`;
  }
...
const MEDIA_CANONICAL={
  ...
  student_research_and_conference_presentations:"student_videos_and_conference_presentations",
  ...
  student_practical_training:"student_teaching_and_practical_training_activities",
  ...
  stem_laboratory:"3d_printing_and_stem_laboratory_activities",
  ...
  maintenance_troubleshooting:"technical_maintenance_and_troubleshooting_work",
  graduation_highlights:"graduation_highlights",
  graduation:"graduation_highlights",
  other:"other"
};
function canonicalMediaCategory(value){
  const key=String(value||"other").toLowerCase().trim();
  return MEDIA_CANONICAL[key]||key||"other";
}
...
const FINAL_MEDIA_LABELS={
  student_videos_and_conference_presentations:"Student Videos and Conference Presentations",
  student_teaching_and_practical_training_activities:"Student Teaching and Practical Training Activities",
  teacher_training_and_professional_development_programs:"Teacher Training and Professional Development Programs",
  "3d_printing_and_stem_laboratory_activities":"3D Printing and STEM Laboratory Activities",
  technical_maintenance_and_troubleshooting_work:"Technical Maintenance and Troubleshooting Work",
...
function finalMediaLabel(key){ return FINAL_MEDIA_LABELS[key]||mediaLabel(key); }
...
async function renderFinalMediaLibrary(){
  ...
  rows.forEach(item=>{
    const original=String(item.row.category||"other").toLowerCase();
    if(original==="institutional_evidence") return;
    const key=canonicalMediaCategory(original);
  ...
  const storageCount=Number(data._storageFileCount||0);
  const assetCount=(data.assets||[]).length;
  const unlinkedStorage=(data._unlinkedStorageFiles||[]).length;
  mount.innerHTML=`
    ...
    <div class="asset-audit-strip"><span><strong>${storageCount||assetCount}</strong>Appwrite Storage files</span><span><strong>${rows.length}</strong>media records</span><span><strong>${imageCount}</strong>images</span><span><strong>${videoCount}</strong>videos</span><span><strong>${fileCount}</strong>documents / presentations</span></div>
    ${unresolved?`<div class="data-warning"><strong>${unresolved} media record${unresolved===1?"":"s"} could not be matched to a public Storage file.</strong> The record remains visible rather than being silently removed, so its relationship can be corrected safely.</div>`:""}
    ${unlinkedStorage?`<div class="data-warning"><strong>${unlinkedStorage} Storage file${unlinkedStorage===1?" is":"s are"} not represented by an asset record.</strong> These files are not guessed into a category because doing so could place academic evidence in the wrong section. They should be linked explicitly in Appwrite.</div>`:""}
    ${keys.length?keys.map((key,sectionIndex)=>{
      const items=byCategory.get(key)||[];
      return `<section class="media-category-section" id="media-${escapeAttr(key)}">
        <div class="media-category-head"><div><p class="section-label">Collection ${String(sectionIndex+1).padStart(2,"0")}</p><h3>${escapeHTML(finalMediaLabel(key))}</h3><p>${escapeHTML(FINAL_MEDIA_DESCRIPTIONS[key]||"Curated professional and academic media associated with this activity area.")}</p></div><span class="media-category-count">${items.length} item${items.length===1?"":"s"}</span></div>
  ...
  mount.innerHTML=`<div class="evidence-library">${rows.map(r=>{
    ...
    return `<article class="evidence-academic-card">
      ...
      <div class="evidence-copy">
        <span class="evidence-label">Academic context · ${escapeHTML(prettyCategory(r.evidence_type||"Institutional Evidence"))}</span>
        ...
        <div class="evidence-source-block"><span class="evidence-source-label">Direct link to the original post</span>${source?`<a class="evidence-source-link" href="${escapeAttr(source)}" target="_blank" rel="noopener">${escapeHTML(source)} ↗</a>`:`<span class="evidence-date">Direct source URL not yet attached to this record.</span>`}</div>
      </div>
    </article>`;
  }).join("")}</div>`;
}
